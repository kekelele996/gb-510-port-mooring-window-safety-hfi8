  ...
  return defineStore(id, {
    ...
    actions: {
      ...
      async confirmClearance(path: string, item: DomainRecord) {
        this.loading = true;
        this.error = '';
        // Always re-read the row first: the window may have changed version or
        // become unsafe after the panel was rendered.
        try {
          const fresh = await request<DomainRecord>(`/${path}/${item.id}`);
          const current = fresh.data;
          await request<DomainRecord>(`/${path}/${item.id}/transition`, {
            method: 'POST',
            body: JSON.stringify({
              status: 'cleared', expectedVersion: current.version,
              windowVersion: current.windowVersion || 1, reason: '已复核风浪窗口安全状态与版本、缆绳方案和回退措施',
            }),
          });
          await this.load(path);
        } catch (error) {
