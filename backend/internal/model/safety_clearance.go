...
type SafetyClearance struct {
	...
	// RebindRequired marks a submitted pending clearance whose bound window moved
	// to a new version. The original submitter is preserved, but the old
	// submission cannot be released until the submitter re-confirms it.
	RebindRequired bool       `json:"rebindRequired" gorm:"not null;default:false"`
